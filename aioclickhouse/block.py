from aioclickhouse.reader import read_varint, read_binary_uint8, read_binary_int32
from aioclickhouse.writer import write_varint, write_binary_uint8, write_binary_int32
...
class BlockInfo:
    ...
    async def read(self, buf):
        while True:
            field_num = await read_varint(buf)
            if not field_num:
                break

            if field_num == 1:
                self.is_overflows = bool(await read_binary_uint8(buf))

            elif field_num == 2:
                self.bucket_num = await read_binary_int32(buf)
