...
class ArrayColumn(Column):
    ...
    async def read_data(self, rows, buf):
        self.nested_column = ArrayColumn(self.nested_column)
        self.nested_column.nullable = self.nullable
        self.nullable = False
        return await self._read(rows, buf)
    ...
    async def _read(self, size, buf):
        ...
        if self.nested_column.nullable:
            nulls_map = await self._read_nulls_map(size, buf)
        else:
            nulls_map = [0] * size
        ...
        while not q.empty():
            ...
            if cur_depth != depth:
                ...
                if nested_column.nullable:
                    nulls_map = await self._read_nulls_map(prev_offset, buf)
                else:
            ...
            # Read data
                ...
                data.extend(
                    await nested_column._read_data(
                        size, buf,
