...
from ..exceptions import TooLargeStringSize
...
__all__ = ['String', 'create_fixed_string_column']
...
class String(CustomItemColumn):
    ...
    async def read(self, buf):
        return self.try_decode(await read_binary_bytes(buf))
...
class FixedString(String):
    ...
    async def read(self, buf):
        text = self.try_decode(await read_binary_bytes_fixed_len(buf, self.length))
